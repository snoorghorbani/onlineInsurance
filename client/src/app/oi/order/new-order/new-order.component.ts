...
export class NewOrderComponent implements OnInit {
	...
	compare() {
		from([ this.formGroup ]).combineLatest(this.orderForm$).subscribe(([ formGroup, orderForm ]) => {
			Object.keys(formGroup.value).forEach(key => (orderForm[key].Value = formGroup.value[key]));
			this.store.dispatch(new ComparePoliciesStartAction(orderForm as ComparePoliciesApiModel.Request));
		});
	}
}
