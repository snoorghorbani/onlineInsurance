...
export const httpPost = (endpoint: string, body: any): Observable<any> => {
	return Observable.create((observer: any) => {
	...
	if (this.is.array(objOrArr)) return objOrArr[pathOrIndex];
		...
		for (let i = 0, route; (route = routes[i]); i++) {
			...
			if (this.is.array(tempobjOrArr[route])) {
				...
				return res;
			} else tempobjOrArr = tempobjOrArr[route];
		}
	}
	return tempobjOrArr;
};
...
export const is = (function(_: any) {
	...
	is.empty = function(o: any) {
		if (_.is.object(0)) for (const i in o) if (o.hasOwnProperty(i)) return false;
